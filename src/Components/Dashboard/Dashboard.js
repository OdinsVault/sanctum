  ...
  setComponents = async (user) => {
    ...
    if (user.Role[0] === 'user') {
      try {
        ...
        let course = getCoursesList.overview.length < 5 ? getCoursesList.overview : getCoursesList.overview.slice(0, 5)
        let practice = getPracticeList.questions.length < 5 ? getPracticeList.questions : getPracticeList.questions.slice(0, 5)
        let compete = getCompetitions.questions.length < 5 ? getCompetitions.questions : getCompetitions.questions.slice(0, 5)

        this.setState({
          courses: course,
          practicalList: practice,
          competitionList: compete,
          dataLoading: false
        })
          ...
          <Row gutter={16}>
            ...
            <Col span={8}>
              <Card title={<b><span>Practice</span></b>}
                ...
                <Spin spinning={this.state.dataLoading}>
                  {this.state.practicalList ? this.state.practicalList.map((practicle) => (
            ...
            <Col span={8}>
              <Card title={<b><span>Compete</span></b>}
                ...
                <Spin spinning={this.state.dataLoading}>{
                  this.state.competitionList ? this.state.competitionList.map((compete) => (
                    <Card.Grid key={compete._id} style={{
                      width: '100%',
                      backgroundColor: '#e3ffd9'
                    }}>{compete.title}</Card.Grid>
                  )) : COMPETE_DESCRIPTION
                }
                </Spin>
              </Card>
